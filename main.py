...
class Config:
    def __init__(self):
        ...
        try:
            ...
            print(f"✅ Директории созданы: {self.BACKUP_DIR}, {self.LOGS_DIR}")
...
# =========== БАЗА ДАННЫХ ===========
class Database:
    def __init__(self, db_path: str = "/app/tenders.db"):
        ...
        try:
            async with aiosqlite.connect(self.db_path) as conn:
                ...
                CREATE TABLE IF NOT EXISTS questionnaires (
                    ...
                    UNIQUE(user_id, full_name, company_name, inn)
                )
                ''')
...
async def create_backup():
    ...
    try:
        if not os.path.exists(config.DB_PATH):
            logger.warning(f"База данных не найдена по пути: {config.DB_PATH}")
            return None
